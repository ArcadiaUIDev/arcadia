...
public abstract class FieldBase : Core.Base.HelixComponentBase
{
    ...
    /// </summary>
    protected string? ContainerClass => CssBuilder.Default("helix-field")
        .AddClass("helix-field--error", HasErrors)
        .AddClass("helix-field--disabled", EffectiveDisabled)
        .AddClass("helix-field--required", EffectiveRequired)
        .AddClass(Class)
        .Build();
}
